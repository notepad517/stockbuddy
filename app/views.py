...
def index(request):
    ...
    aaplpd = pd.DataFrame(df1)
    pred_fig = go.Figure([go.Scatter(x=aaplpd['Volume'], y=aaplpd['Open'])])
    pred_fig.update_xaxes(rangeslider_visible=True)
    pred_fig.update_layout(paper_bgcolor="#66b3ff", plot_bgcolor="#ff8080", font_color="white")
    plot_div_pred = plot(pred_fig, auto_open=False, output_type='div')
    # Extracting AAPL data from fig_left
    aapl_data = data['AAPL']['Adj Close']

    # Creating the sample_graph with AAPL data
    sample_graph = go.Figure(go.Scatter(x=data['Date'], y=aapl_data))
    sample_graph.update_layout(
        paper_bgcolor="#ff4d4d",
        plot_bgcolor="#ccccff",  
        font_color="white",
        height=800,
        legend=dict(
